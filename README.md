...
Primeiro, verifica se a entrada é `None`. Depois, o `re.sub()` remove caracteres que não sejam letras ou números e o `.lower()` transforma as letras em minúsculas. Em seguida, a string é invertida usando `[::-1]`. Por último, o programa compara as duas strings e retorna `True` se forem iguais ou `False` se forem diferentes.
